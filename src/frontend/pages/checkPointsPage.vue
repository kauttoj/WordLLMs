          ...
          >
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                ...
                <span v-if="item.mode" class="rounded bg-accent/10 px-1.5 py-0.5 text-xs text-accent">
                  {{ item.mode }}
                </span>
                <span v-if="item.messageCount" class="text-xs text-secondary">
...
import { deleteThreadFromBackend, fetchThreadList } from '@/api/backend'
...
const loadAllSessions = async () => {
  ...
  try {
    ...
    sessionItems.value = threads.map(thread => {
      ...
      return {
        ...
        previewText: previewText || '[Empty Conversation]',
    ...
    console.log('[CheckPoints] Loaded', sessionItems.value.length, 'conversations')
  } catch (err) {
    console.error('[CheckPoints] CRITICAL: Failed to load conversations:', err)
    throw err
  } finally {
    loading.value = false
  }
...
const deleteSession = async (threadId: string) => {
  try {
    await deleteThreadFromBackend(threadId)
